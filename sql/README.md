...
| `mark_past_dilution.sql` | **Changes data.** Marks AZEI's Sep 3 dilution on its price point (and in every snapshot holding it), which predates the mark — without it JXI's 1M/Max chart shows a ~9% drop on Sep 3 that never happened. Marks only when exactly one point matches the dilution's step; otherwise skips and reports. Moves no money, shares or price. |
| `push_notification_guard.sql` | Any signed-in account could send any text to any student and have it **emailed** from the exchange's account. Email from a non-officer to someone else now carries no student-written text; `session`/`ipo`/`minutes`/`price_adj` are officer-only; `bug_report`/`flag`/`contact_admin` go to officers only; rate limited; the sender is recorded. Also: `margin_call` was never on the allowed list, so **no margin-call notification was ever delivered**. |
| `preflight.sql` | **Read-only, run it before class.** Answers "is the exchange in a state where a lesson can happen" — dev_mode, session status, whether the books balance, stranded prices, shorts near their margin line, snapshot freshness, whether 23 named fixes are still present, and three data checks: no function writing UTC times, no dilution the index chart would draw as a crash, and a JXI open that matches its constituents. |
